...
(function () {
    ...
    function setupMediaPreview() {
        ...
        // 1. FIRST PRIORITY:
        // Use the currently selected Post photo.
        // This keeps Trim preview synced with the active photo.
        try {
            const currentIndex =
                (typeof window.postEditCurrentIndex === "number")
                    ? window.postEditCurrentIndex
                    : 0;

            const currentFiles =
                (typeof postEditFiles !== "undefined" && postEditFiles && postEditFiles.length)
                    ? postEditFiles
                    : ((typeof selectedFiles !== "undefined" && selectedFiles && selectedFiles.length)
                        ? selectedFiles
                        : null);

            if (currentFiles && currentFiles[currentIndex]) {
                const currentFile = currentFiles[currentIndex];

                mediaSrc = URL.createObjectURL(currentFile);
