/* =====================================================
   SNAPZ REELS TAG PEOPLE (ISOLATED)
===================================================== */
(function() {
    console.log("Loading isolated Reel Tagging...");

    window.snapzOpenReelTagPeople = function() {
        console.log("Reel Tag Modal Opened!");
        
        let modal = document.getElementById("snapzReelTagModal");
        if (!modal) {
            modal = document.createElement("div");
            modal.id = "snapzReelTagModal";
            ...
            modal.innerHTML = `
                <div style="background:#1e1e1e;color:#fff;width:90%;max-width:400px;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.5);">
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:14px 16px;border-bottom:1px solid #333;">
                        ...
                        <button type="button" id="snapzReelCloseModal" style="background:none;border:none;color:#aaa;font-size:20px;cursor:pointer;">&times;</button>
                    </div>
                    <div style="padding:12px;">
                        <input type="text" id="snapzReelSearchInput" placeholder="Search users..." style="width:100%;padding:10px 12px;background:#2a2a2a;border:1px solid #444;color:#fff;border-radius:6px;box-sizing:border-box;font-size:14px;outline:none;">
                    </div>
                    <div id="snapzReelUserList" style="max-height:250px;overflow-y:auto;padding:0 16px 16px;">
            ...
            document.getElementById("snapzReelCloseModal").onclick = () => { modal.style.display = "none"; };
            modal.onclick = (e) => { if (e.target === modal) modal.style.display = "none"; };

            document.getElementById("snapzReelSearchInput").oninput = function() {
                const query = this.value.trim();
                const container = document.getElementById("snapzReelUserList");
    ...
    document.addEventListener("click", function(e) {
        const btn = e.target.closest("#snapzReelTagPeopleButton");
        if (btn) {
            e.preventDefault();
            e.stopPropagation();
            window.snapzOpenReelTagPeople();
        }
    }, true);
})();
